...
test('extracts multiple variables', () => {
  ...
  expect(reverseEjs(template, final)).toEqual({ firstName: 'John', lastName: 'Doe' })
})

// ─── 3. Simple loop — string array ───────────────────────────────────────────
test('extracts a simple loop as a string array', () => {
  const template = '<% items.forEach(item => { %><li><%= item %></li><% }) %>'
  const final    = '<li>Apple</li><li>Banana</li><li>Cherry</li>'
  expect(reverseEjs(template, final)).toEqual({ items: ['Apple', 'Banana', 'Cherry'] })
})

// ─── 4. Loop with object items ────────────────────────────────────────────────
test('extracts a loop as an object array', () => {
  const template = '<% users.forEach(user => { %><li><%= user.name %> - <%= user.age %></li><% }) %>'
...
test('extracts variables and a loop together', () => {
  const template = '<h1><%= title %></h1><% items.forEach(item => { %><li><%= item %></li><% }) %>'
...
test('works with plain text templates', () => {
  ...
  expect(reverseEjs(template, final)).toEqual({ name: 'John', orderId: '#1234' })
})

// ─── 8. Empty loop ────────────────────────────────────────────────────────────
test('returns an empty array for a loop with zero iterations', () => {
  const template = '<ul><% items.forEach(item => { %><li><%= item %></li><% }) %></ul>'
  const final    = '<ul></ul>'
...
test('handles multiline templates', () => {
  ...
  expect(reverseEjs(template, final)).toEqual({ name: 'John', age: '30', city: 'New York' })
})
...
test('handles values containing special characters', () => {
  ...
  expect(reverseEjs(template, final)).toEqual({ message: 'Hello & welcome to <the> party!' })
})
...
test('extracts loop items with four properties each', () => {
  const template =
    ...
    '<% }) %>'
  const final =
    '<div>1,Widget,9.99,100</div>' +
    '<div>2,Gadget,24.99,50</div>'
  expect(reverseEjs(template, final)).toEqual({
    products: [
...
test('<%- raw tag: extracts a single unescaped variable', () => {
  ...
  expect(reverseEjs(template, final)).toEqual({ content: '<b>bold text</b>' })
})
...
test('<%- raw tag: inside a loop', () => {
  const template = '<% items.forEach(item => { %><li><%- item.html %></li><% }) %>'
...
test('<%_: leading whitespace before scriptlet is stripped from output', () => {
  const template = '    <%_ items.forEach(item => { %><li><%= item %></li><%_ }) %>'
...
test('_%>: trailing whitespace after scriptlet is stripped from output', () => {
  const template = '<% items.forEach(item => { _%>   <li><%= item %></li><% }) %>'
  const final    = '<li>One</li><li>Two</li>'
  expect(reverseEjs(template, final)).toEqual({ items: ['One', 'Two'] })
})
...
test('HTML escaping: ampersands and quotes are unescaped', () => {
  ...
  expect(reverseEjs(template, final)).toEqual({ description: 'AT&T "wireless"' })
})

test('HTML escaping: values with multiple entity types', () => {
  const template = '<% items.forEach(item => { %><li><%= item.label %></li><% }) %>'
  const final    = '<li>a &lt; b</li><li>x &amp; y</li><li>&quot;quoted&quot;</li>'
  expect(reverseEjs(template, final)).toEqual({
    items: [
      { label: 'a < b'      },
      { label: 'x & y'      },
...
test('for...of: extracts a simple string array', () => {
  const template = '<% for (const item of items) { %><li><%= item %></li><% } %>'
  const final    = '<li>Alpha</li><li>Beta</li><li>Gamma</li>'
  expect(reverseEjs(template, final)).toEqual({ items: ['Alpha', 'Beta', 'Gamma'] })
})
...
test('if block: extracts variable when condition was true', () => {
  ...
  expect(reverseEjs(template, final)).toEqual({ 'user.name': 'Alice', user: true })
})
...
test('if/else block: extracts from the matching branch', () => {
  ...
  expect(reverseEjs(template, finalTrue)).toEqual({ name: 'Alice', isAdmin: true })
  expect(reverseEjs(template, finalFalse)).toEqual({ name: 'Bob',  isAdmin: false })
})
...
test('two independent if/else conditions with literal-only branches', () => {
  ...
  expect(reverseEjs(template, '✓ Alice (online)')).toEqual({
    isVerified: true, username: 'Alice', isOnline: true,
  })
  expect(reverseEjs(template, '✗ Bob (offline)')).toEqual({
    isVerified: false, username: 'Bob', isOnline: false,
  })
  expect(reverseEjs(template, '✓ Carol (offline)')).toEqual({
    isVerified: true, username: 'Carol', isOnline: false,
  })
})
...
test('if/else: extra variable only present in the then branch', () => {
  ...
  expect(reverseEjs(template, '<span class="badge">Gold</span> Alice')).toEqual({
    hasBadge: true, badge: 'Gold', name: 'Alice',
  })
  expect(reverseEjs(template, '<span class="no-badge"></span> Bob')).toEqual({
    hasBadge: false, name: 'Bob',
  })
})
...
test('nested if/else: two levels deep, inner condition only when outer is true', () => {
  ...
  expect(reverseEjs(template, '<p>Admin: Alice</p>')).toEqual({
    isLoggedIn: true, isAdmin: true, name: 'Alice',
  })
  expect(reverseEjs(template, '<p>User: Bob</p>')).toEqual({
    isLoggedIn: true, isAdmin: false, name: 'Bob',
  })
...
test('if/else: loop inside then branch, no loop data when else runs', () => {
  ...
  expect(reverseEjs(template, '<ul><li>Alpha</li><li>Beta</li></ul>')).toEqual({
    hasItems: true, items: ['Alpha', 'Beta'],
  })
...
test('if/else: same variable names in both branches resolve to one value each', () => {
  ...
  expect(reverseEjs(template, 'Alice Smith — Premium')).toEqual({
    isPaid: true, firstName: 'Alice', lastName: 'Smith',
  })
  expect(reverseEjs(template, 'Bob Jones — Free')).toEqual({
    isPaid: false, firstName: 'Bob', lastName: 'Jones',
  })
})
...
test('include: extracts a variable defined inside a partial', () => {
  ...
  expect(reverseEjs(template, final, { partials })).toEqual({ name: 'Alice' })
})
...
test('include: loop inside partial is fully extracted', () => {
  ...
  const final    = '<h2>Fruits</h2><ul><li>Apple</li><li>Banana</li><li>Cherry</li></ul>'
  expect(reverseEjs(template, final, { partials })).toEqual({
    heading: 'Fruits',
...
test('include: partial with object-array loop extracts item properties', () => {
  ...
  const final    = '<ul><li>Alice &lt;alice@example.com&gt;</li><li>Bob &lt;bob@example.com&gt;</li></ul>'
  expect(reverseEjs(template, final, { partials })).toEqual({
    users: [
...
test('include: same partial included twice enforces consistent variable values', () => {
  ...
  const template =
    ...
    '<%- include("divider") %>'  +
    '<%= outro %>'
  const final = 'Top<hr data-style="thick">Middle<hr data-style="thick">Bottom'
  expect(reverseEjs(template, final, { partials })).toEqual({
    intro:        'Top',
    dividerStyle: 'thick',
    content:      'Middle',
...
test('include: locals argument syntax is accepted without breaking extraction', () => {
  ...
  expect(reverseEjs(template, final, { partials })).toEqual({ cardTitle: 'Welcome' })
})
...
test('include: throws a descriptive error when the referenced partial is absent', () => {
  ...
  expect(() => reverseEjs(template, '<p>Before</p>anything<p>After</p>', { partials: {} }))
...
test('else if: extracts variable from whichever branch matched', () => {
  ...
  expect(reverseEjs(template, '<p class="admin">Alice</p>')).toEqual({ name: 'Alice' })
  expect(reverseEjs(template, '<p class="mod">Bob</p>')).toEqual({ name: 'Bob' })
  expect(reverseEjs(template, '<p class="user">Carol</p>')).toEqual({ name: 'Carol' })
})
...
test('else if: simple-identifier conditions become booleans in the result', () => {
  ...
  expect(reverseEjs(template, '<span>Basic</span>')).toEqual({
    isPremium: false, isBasic: true,
  })
  expect(reverseEjs(template, '<span>Free</span>')).toEqual({
    isPremium: false, isBasic: false,
  })
})
...
test('switch/case: extracts variable from the matching case branch', () => {
  ...
  expect(reverseEjs(template, '<span class="green">Running</span>')).toEqual({ label: 'Running' })
  expect(reverseEjs(template, '<span class="amber">Waiting</span>')).toEqual({ label: 'Waiting' })
  expect(reverseEjs(template, '<span class="red">Failed</span>')).toEqual({ label: 'Failed' })
})
...
test('custom delimiter: ? produces <? ?> syntax — single variable', () => {
  const template = '<?= name ?>'
  const final    = 'Alice'
  expect(reverseEjs(template, final, { delimiter: '?' })).toEqual({ name: 'Alice' })
})
...
test('custom openDelimiter and closeDelimiter', () => {
  const template = '[%= name %]'
  const final    = 'Alice'
  expect(reverseEjs(template, final, {
    openDelimiter:  '[',
    closeDelimiter: ']',
  })).toEqual({ name: 'Alice' })
})
...
test('adjacent variables with no separator: throws a descriptive error', () => {
  ...
  expect(() => reverseEjs(template, final))
    .toThrow('Adjacent variables with no literal separator are ambiguous')
})
...
test('rmWhitespace: template with indentation matches output produced by rmWhitespace:true', () => {
  ...
  const final = '<div>\nAlice\n</div>'
  expect(reverseEjs(template, final, { rmWhitespace: true })).toEqual({ name: 'Alice' })
})
...
test('custom unescape: applies provided function instead of built-in HTML entity map', () => {
  ...
  const unescape = (s: string) =>
    s.replace(/&#(\d+);/g, (_: string, code: string) => String.fromCharCode(Number(code)))
...
test('include: dynamic filename (variable) throws a descriptive error', () => {
  ...
  expect(() => reverseEjs(template, final, { partials: { header: '<header>Nav</header>' } }))
