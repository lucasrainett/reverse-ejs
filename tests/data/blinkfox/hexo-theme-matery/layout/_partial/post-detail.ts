import { createHexoHelpers, createDate, createCollection } from "../../../../helpers";

export default {
	...createHexoHelpers(),
	"config": {
			"title": "Pixel & Code",
			"author": "Alice Chen",
			"url": "https://blog.example.com",
			"root": "/",
			"language": "en",
			"description": "A blog about web development and design",
			"keywords": "blog, web, javascript, hexo",
			"date_format": "YYYY-MM-DD",
			"subtitle": "Thoughts on code and creativity",
			"prismjs": {
				"enable": true
			}
		},
		"theme": {
			"jsDelivr": {
				"url": "https://cdn.jsdelivr.net/gh/user/blog@latest"
			},
			"libs": {
				"css": {
					"fontAwesome": "/libs/awesome/css/all.min.css",
					"materialize": "/libs/materialize/materialize.min.css",
					"aos": "/libs/aos/aos.css",
					"animate": "/libs/animate/animate.min.css",
					"lightgallery": "/libs/lightGallery/css/lightgallery.min.css",
					"matery": "/css/matery.css",
					"mycss": "/css/my.css",
					"gitalk": "/libs/gitalk/gitalk.css",
					"aplayer": "/libs/aplayer/APlayer.min.css",
					"dplayer": "/libs/dplayer/DPlayer.min.css",
					"tocbot": "/libs/tocbot/tocbot.css",
					"prism": "/libs/prism/prism.css",
					"jqcloud": "/libs/jqcloud/jqcloud.min.css"
				},
				"js": {
					"jquery": "/libs/jquery/jquery.min.js",
					"materialize": "/libs/materialize/materialize.min.js",
					"masonry": "/libs/masonry/masonry.pkgd.min.js",
					"aos": "/libs/aos/aos.js",
					"scrollProgress": "/libs/scrollprogress/scrollProgress.min.js",
					"lightgallery": "/libs/lightGallery/js/lightgallery-all.min.js",
					"matery": "/js/matery.js",
					"clicklove": "/libs/others/clicklove.js",
					"busuanzi": "/libs/others/busuanzi.pure.mini.js",
					"canvas_nest": "/libs/others/canvas-nest.min.js",
					"ribbon_dynamic": "/libs/others/ribbon-dynamic.js",
					"instantpage": "/libs/others/instantpage.js",
					"ribbon": "/libs/others/ribbon.min.js",
					"ribbonRefresh": "/libs/others/ribbon-refresh.min.js",
					"gitalk": "/libs/gitalk/gitalk.min.js",
					"valine": "/libs/valine/Valine.min.js",
					"tocbot": "/libs/tocbot/tocbot.min.js",
					"aplayer": "/libs/aplayer/APlayer.min.js",
					"dplayer": "/libs/dplayer/DPlayer.min.js",
					"echarts": "/libs/echarts/echarts.min.js",
					"jqcloud": "/libs/jqcloud/jqcloud.min.js",
					"crypto": "/libs/crypto-js/crypto-js.min.js"
				}
			},
			"postInfo": {
				"date": true,
				"update": true,
				"wordCount": true,
				"min2read": true,
				"readCount": true,
				"totalCount": true
			},
			"busuanziStatistics": {
				"enable": true,
				"totalTraffic": true,
				"totalNumberOfvisitors": true
			},
			"code": {
				"lang": true,
				"copy": true,
				"shrink": true,
				"break": false
			},
			"copyright": {
				"enable": true,
				"minCharNumber": 120,
				"description": "Please include the original link when reprinting."
			},
			"reward": {
				"enable": true,
				"title": "Buy me a coffee",
				"alipay": "/medias/reward/alipay.png",
				"wechat": "/medias/reward/wechat.png"
			},
			"reprint": {
				"enable": true,
				"default": "cc_by"
			},
			"gitalk": {
				"enable": true,
				"owner": "alicechen",
				"repo": "blog-comments",
				"admin": [
					"alicechen"
				],
				"oauth": {
					"clientId": "abc123gitalk",
					"clientSecret": "secret123gitalk"
				}
			},
			"gitment": {
				"enable": false,
				"owner": "alicechen",
				"repo": "blog-comments",
				"oauth": {
					"clientId": "abc123gitment",
					"clientSecret": "secret123gitment"
				}
			},
			"disqus": {
				"enable": false,
				"shortname": "pixelandcode"
			},
			"livere": {
				"enable": false,
				"uid": "MTAyMC8zNjQ5Ny8xNjEzMQ=="
			},
			"valine": {
				"enable": false,
				"appId": "valine_app_id_123",
				"appKey": "valine_app_key_456",
				"notify": false,
				"verify": false,
				"visitor": true,
				"avatar": "mp",
				"pageSize": 10,
				"placeholder": "Leave a comment...",
				"background": "/medias/comment-bg.png"
			},
			"minivaline": {
				"enable": false
			},
			"changyan": {
				"enable": false,
				"appId": "changyan_app_id",
				"conf": "changyan_conf_123"
			},
			"sharejs": {
				"enable": true,
				"sites": "wechat,qq,weibo,tencent,twitter,facebook"
			},
			"addthis": {
				"enable": false,
				"pubid": "ra-5c5c5c5c5c5c5c5c"
			},
			"bbtime": {
				"enable": false
			},
			"featureImages": [
				"/medias/featureimages/0.jpg",
				"/medias/featureimages/1.jpg",
				"/medias/featureimages/2.jpg",
				"/medias/featureimages/3.jpg"
			]
		},
		"page": {
			"title": "Hello World",
			"content": "<p>This is a sample blog post with some interesting content about web development.</p>",
			"date": "2025-01-15",
			"updated": "2025-01-16",
			"tags": [
				{
					"name": "Node.js",
					"path": "tags/nodejs/",
					"length": 4
				},
				{
					"name": "Tutorial",
					"path": "tags/tutorial/",
					"length": 6
				}
			],
			"categories": [
				{
					"name": "JavaScript",
					"path": "categories/javascript/",
					"length": 5
				}
			],
			"excerpt": "This is a sample blog post with some interesting content.",
			"summary": "A brief overview of the Hello World post.",
			"permalink": "https://blog.example.com/posts/hello-world/",
			"path": "posts/hello-world/",
			"photos": [],
			"toc": true,
			"img": "/medias/featureimages/1.jpg",
			"description": "Page not found",
			"author": "Alice Chen",
			"password": "",
			"mathjax": false,
			"layout": "post",
			"reprintPolicy": "cc_by",
			"current": 1,
			"total": 3,
			"prev": {
				"title": "Previous Post Title",
				"path": "posts/previous-post/",
				"img": "/medias/featureimages/0.jpg",
				"date": "2025-01-10",
				"excerpt": "Previous post excerpt content goes here.",
				"summary": "Summary of the previous post.",
				"content": "<p>Previous post content here.</p>",
				"author": "Alice Chen",
				"categories": [
					{
						"name": "JavaScript",
						"path": "categories/javascript/"
					}
				],
				"tags": [
					{
						"name": "Node.js",
						"path": "tags/nodejs/"
					}
				]
			},
			"next": {
				"title": "Next Post Title",
				"path": "posts/next-post/",
				"img": "/medias/featureimages/2.jpg",
				"date": "2025-01-20",
				"excerpt": "Next post excerpt content goes here.",
				"summary": "Summary of the next post.",
				"content": "<p>Next post content here.</p>",
				"author": "Alice Chen",
				"categories": [
					{
						"name": "Web Dev",
						"path": "categories/web-dev/"
					}
				],
				"tags": [
					{
						"name": "CSS",
						"path": "tags/css/"
					}
				]
			},
			"prev_link": "/page/1/",
			"next_link": "/page/3/",
			"year": 2025,
			"month": 1,
			"category": "JavaScript",
			"tag": "Node.js",
			"__post": true,
			"keywords": "hexo, blog, javascript"
		},
};
