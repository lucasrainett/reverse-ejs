...
import { EjsOptions, ExtractedObject } from './types'
...
function expandIncludes(
  ...
  if (depth > 20) throw new Error('Include depth limit exceeded — possible circular include')
  return template.replace(INCLUDE_RE, (_match, name: string) => {
    const partial = partials[name]
    if (partial === undefined) throw new Error(`Partial "${name}" not found`)
