  ...
  switch (pattern.type) {
    ...
    case 'conditional': {
      ...
      if (!pattern.elseBranch) {
        ...
        const thenRegex = buildRegex(pattern.thenBranch, itemName, thenSeen, tSfx, loopVar)
      ...
      const elseRegex = buildRegex(pattern.elseBranch, itemName, elseSeen, eSfx, loopVar)
      for (const k of thenSeen) seen.add(k)
    ...
    case 'sequence': {
      ...
      return pattern.parts
        .map(p => buildRegex(p, itemName, seen, branchSuffix, loopVar))
        .join('')
    }
  }
}
...
export function buildRegexNoGroups(pattern: Pattern): string {
  switch (pattern.type) {
    case 'literal':       return escapeRegex(pattern.value)
    case 'variable':      return `[\\s\\S]+?`
    case 'loop': {
