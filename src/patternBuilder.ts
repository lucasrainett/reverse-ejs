...
function parseSequence(
  ...
  while (i < tokens.length) {
    ...
    // ── Conditional ──────────────────────────────────────────────────────────
      ...
      const [thenBranch, stopIdx, stopReason] = parseSequence(tokens, i + 1)

      if (stopReason === 'else') {
        const [elseBranch, endIdx] = parseSequence(tokens, stopIdx + 1)
        parts.push({ type: 'conditional', thenBranch, elseBranch, condition: token.condition })
        i = endIdx + 1
      } else {
        parts.push({ type: 'conditional', thenBranch, condition: token.condition })
        i = stopIdx + 1
      }
