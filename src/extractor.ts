import { Pattern, LoopPattern, ExtractedObject, ExtractedItem, EjsOptions } from './types'
...
function groupsToObject(
  ...
  for (const captureName of Object.keys(groups)) {
    ...
    if (captureName.endsWith('_LOOP')) {
      ...
      if (loopPattern) {
        result[arrayName] = extractLoopItems(loopPattern, value, unescape)
      }
...
function extractLoopItems(
  ...
  while ((match = bodyRegex.exec(loopSection)) !== null) {
    ...
    const allKeys = [...Object.keys(simpleGroups), ...Object.keys(nestedLoops)]
    ...
    if (isSimple) {
      ...
      for (const [encodedName, content] of Object.entries(nestedLoops)) {
        ...
        const nested    = findLoop(loopPattern.body, arrayName)
                       ?? findLoop(loopPattern.body, `${loopPattern.itemName}.${arrayName}`)
        if (nested) item[arrayName] = extractLoopItems(nested, content, unescape)
      }
      items.push(item)
    }
  }
...
function extractConditionBooleans(
  ...
  if (pattern.type === 'conditional') {
    ...
    if (pattern.elseBranch) extractConditionBooleans(groups, pattern.elseBranch, result)
  } else if (pattern.type === 'sequence') {
    for (const part of pattern.parts) extractConditionBooleans(groups, part, result)
...
function findLoop(pattern: Pattern, arrayName: string): LoopPattern | null {
  if (pattern.type === 'loop') {
    if (pattern.arrayName === arrayName || pattern.arrayName.endsWith(`.${arrayName}`)) {
      return pattern
    }
